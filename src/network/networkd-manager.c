...
int manager_new(Manager **ret) {
        ...
        LIST_HEAD_INIT(m->swports);
        LIST_HEAD_INIT(m->default_swports);
...
static int manager_rtnl_process_link(sd_rtnl *rtnl, sd_rtnl_message *message, void *userdata) {
        ...
        case RTM_NEWLINK:
                if (!link) {
                        ...
                        /* check to see if it's a switchport, and if so apply its config */
                        LIST_FOREACH(swports,swport,m->swports) {
                                if(strcmp(swport->match_name, link->ifname) == 0)
                                {
                                        r = swport_apply_attributes(swport);
                                        if (r < 0 ) {
