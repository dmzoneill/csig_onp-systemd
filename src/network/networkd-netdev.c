...
static int netdev_load_one(Manager *manager, const char *filename) {
        ...
        if (!netdev->mac) {
                if(netdev->kind == NETDEV_KIND_VLAN) {
                    /* Do not generate random MAC for VLAN.
                    * Kernel will copy the parent MAC by default */
                    log_debug_netdev(netdev, "Skipping generating MAC address");
                    r = 0;
                } else {
...
int netdev_load(Manager *manager) {
        ...
        r = netdev_team_delete_not_configured(manager);
        if (r < 0)
                return r;
