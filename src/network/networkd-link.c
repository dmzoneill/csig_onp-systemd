...
static int link_set_bridge_fdb(Link *const link) {
        ...
        if (link->network->team) {
                r = link_get(link->manager, link->network->team->ifindex, &m_link);
                if (r < 0 || !m_link) {
                        log_error_link(link, "Failed to get master link: %s", strerror(-r));
                        return 0;
                }
        }
        else
        ...
        r = fdb_entries_clear(m_link->manager->rtnl, m_link->ifindex);
        if (r < 0) {
                log_error_link(m_link, "Failed to clear existing entries: %s", strerror(-r));
                return r;
        }

        if (m_link->network) {

                    LIST_FOREACH(static_fdb_entries, fdb_entry, m_link->network->static_fdb_entries) {
                        r = fdb_entry_configure(m_link, fdb_entry);
                        if (r < 0) {
                                log_error_link(m_link, "Failed to add MAC entry to static MAC table: %s", strerror(-r));
                                break;
                        }
                }
        }
...
static int link_set_team_atts(Link *const link) {
        SWPort *swport;
        Link *m_link;
        int r = 0;
        ...
        if (link->network->team) {
                ...
                /* check to see if it's a physical LAG, and if so apply its config */
                LIST_FOREACH(swports,swport,m_link->manager->swports) {
                        if(strcmp(swport->match_name, m_link->ifname) == 0)
                        {
                                swport_apply_attributes(swport);
                                if (r < 0) {
                                        log_error("Could not set attributes on %s %s", swport->match_name, strerror(-r));
                                }
                                break;
                        }
                }
...
static int link_down_handler(sd_rtnl *rtnl, sd_rtnl_message *m, void *userdata) {
        ...
        if (r < 0)
                log_struct_link(LOG_WARNING, link,
                                "MESSAGE=%-*s: could not bring down interface: %s",
                                IFNAMSIZ,
                                link->ifname, strerror(-r),
                                "ERRNO=%d", -r,
                                NULL);
...
static int link_down(Link *link) {
        ...
        log_debug_link(link, "bringing link down");
...
static int link_enter_join_netdev(Link *link) {
        ...
        if (link->network->team) {
                log_struct_link(LOG_DEBUG, link,
                                ...
                                IFNAMSIZ,
                                link->ifname, link->network->team->ifname,
                                NETDEVIF(link->network->team),
                                NULL);

                r = netdev_join(link->network->team, link, &netdev_join_handler);
                if (r < 0) {
                        log_struct_link(LOG_WARNING, link,
