...
static int load_link(link_config_ctx *ctx, const char *filename) {
        ...
        link->match_phys_port_id = UINT_MAX;
...
static int phys_pid_get_value(const char *attr_value, unsigned int *rval)
{
       long l;
       char *x = NULL;
       ...
       l = strtol(attr_value, &x, 16);
       if (!x || x == attr_value || *x || errno) {
...
int link_config_get(link_config_ctx *ctx, struct udev_device *device,
                    link_config **ret) {
        link_config *link;
        const char *phys_port_id_attr_value;
        unsigned int phys_port_id;
        bool dev_has_port_id;

        phys_port_id_attr_value = udev_device_get_sysattr_value(device, "phys_port_id");
        dev_has_port_id = (phys_pid_get_value(phys_port_id_attr_value, &phys_port_id) == 0);
