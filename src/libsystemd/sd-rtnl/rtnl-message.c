...
int sd_rtnl_message_append_binary(sd_rtnl_message *m, unsigned short type, void *data, size_t _size) {
        int r;
        size_t size;
        ...
        r = message_attribute_has_type(m, type, NLA_BINARY);
        if (r < 0)
                return r;
        else
                size = (size_t)r;
        ...
        r = add_rtattr(m, type, data, _size);
        if (r < 0)
                return r;
...
int sd_rtnl_message_append_u64(sd_rtnl_message *m, unsigned short type, uint64_t data) {
        int r;

        assert_return(m, -EINVAL);
        assert_return(!m->sealed, -EPERM);

        r = message_attribute_has_type(m, type, NLA_U64);
        if (r < 0)
                return r;
...
int sd_rtnl_message_read_binary(sd_rtnl_message *m, unsigned short cont_type, unsigned short type, void *data, size_t size, size_t num) {
        int r, rem;
        size_t i;
        void *attr_data;
        struct rtattr *rta, *rtalist;
        char *_data;
        const NLType *nl_cont_type;
        ...
        r = type_system_get_type(m->container_type_system[m->n_containers - 1],
                                 &nl_cont_type,
                                 cont_type);
        if (r < 0)
                return r;

        if (nl_cont_type->type != NLA_NESTED)
                return -EINVAL;

        if(!m->rta_offset_tb[m->n_containers - 1][cont_type])
                return -ENODATA;

        if(!m->rta_offset_tb[m->n_containers][type])
                return -ENODATA;

        rtalist = (struct rtattr*)((uint8_t *) m->hdr + m->rta_offset_tb[m->n_containers - 1][cont_type]);

        rem = RTA_PAYLOAD(rtalist);

        _data = (char *) data;

        for (i = 0, rta = RTA_DATA(rtalist); i < num && RTA_OK(rta, rem); ++i, rta = RTA_NEXT(rta, rem)) {
                ...
                attr_data = RTA_DATA(rta);

                memcpy(_data, attr_data, size);
                _data += size;
        }
...
int sd_rtnl_message_new_vlan(sd_rtnl *rtnl, sd_rtnl_message **ret, uint16_t nlmsg_type, int index) {
        int r;
        struct ifinfomsg *ifi;
        ...
        if (nlmsg_type == RTM_SETLINK || nlmsg_type == RTM_GETLINK)
